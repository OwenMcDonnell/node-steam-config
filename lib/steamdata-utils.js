...
async function requestOwnedApps (id64, force = false, cache = {enabled: false}) {
  ...
  let rf = async () => {
    ...
    data = await fetchAsync(`https://steamcommunity.com/profiles/${id64}/games/?tab=all&xml=1`).catch((err) => {
      /* istanbul ignore next */
      throw err
    })
...
async function requestTags (/* istanbul ignore next */ force = false,  /* istanbul ignore next */ cache = {}) {
  let rf = async () => {
    let data = await fetchAsync('https://store.steampowered.com/tagdata/populartags/english').catch((err) => {
      /* istanbul ignore next */
      throw err
    })
    return JSON.parse(await data.text().catch((err) => {
      /* istanbul ignore next */
      throw err
    }))
  }
...
async function requestGenres (appid, /* istanbul ignore next */ force = false, /* istanbul ignore next */ cache = {}) {
  ...
  if (!force && cache.enabled && cache.folder && cache.file && fs.existsSync((cacheFile = path.join(cache.folder, cache.file)))) {
    cacheFile = JSON.parse('' + await afs.readFileAsync(cacheFile))
  ...
  if (typeof data === 'undefined') {
    let rf = async () => {
      try {
        ...
        data = await fetchAsync(`http://store.steampowered.com/app/${appid}/`, {
          credentials: 'include',
          headers: {
        ...
        data = '' + await data.text().catch((err) => {
          /* istanbul ignore next */
          throw err
        })
