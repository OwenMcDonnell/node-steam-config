...
async function requestOwnedApps (id64, force = false, cache = {enabled: false}) {
  ...
  let rf = async () => {
    ...
    data = await fetch(`https://steamcommunity.com/profiles/${id64}/games/?tab=all&xml=1`).catch((err) => {
      /* istanbul ignore next */
      throw err
    })
...
async function requestTags (/* istanbul ignore next */ force = false, /* istanbul ignore next */ cache = {}) {
  let rf = async () => {
    let data = await fetch('https://store.steampowered.com/tagdata/populartags/english').catch((err) => {
      /* istanbul ignore next */
      throw err
    })
