...
SteamConfig.prototype.loadApps = function loadApps (apps, entry) {
  ...
  return apps.map(async (f) => {
    ...
    f = await this.load(this.paths.app(tmp, entry)).catch((err) => {
      throw err
    })
    return f
  })
    ...
    for (let entry of entries) {
      ...
      switch (tmp) {
        ...
        case 'appinfo':
          ...
          data = await BVDF.parseAppInfo(data).catch((err) => {
            throw err
          })
        ...
        case 'shortcuts':
          if (fs.existsSync(entry)) {
            data = await BVDF.parseShortcuts(await afs.readFileAsync(entry)).catch((err) => {
              throw err
            })
        ...
        case 'steamapps':
          ...
          data = await Promise.all(data).catch((err) => {
            throw err
          })
