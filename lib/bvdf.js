  ...
  try {
    do {
      ...
      */
      ...
      switch (type) {
        case Type.None:
          object[name] = await decode(buffer).catch((err) => {
    ...
    do {
      try {
        ...
        */
        ...
        let info
        info = await decode(buffer).catch((err) => {
          /* istanbul ignore next */
          throw err
        })
        /*
...
async function parseShortcuts (data) {
  ...
  try {
    data = await decode(ByteBuffer.wrap(data, 'hex', true).resize(data.length)).catch((err) => {
      /* istanbul ignore next */
      throw err
    })
